...
See README.md for setup, score, and data-use notes.
"""
...
def cfg_for(c):
    return CFG["Log" if c.startswith("Log") else "Dipole"]
...
_DIST = np.array(
    [abs(_CO[i][0] - _CO[j][0]) + abs(_CO[i][1] - _CO[j][1]) for i, j in _ALLP]
)
...
def qfeat(Spm):
    return np.hstack(
        [np.ones((len(Spm), 1)), Spm, Spm[:, PAIRS[:, 0]] * Spm[:, PAIRS[:, 1]]]
    )
...
# F-order flattening). config_id 1..16  => four uniform 8x8 blocks (2 bits each axis). config_id 17..528 => nine uniform blocks on a (5,5,6) grid.
def mblock9(n):
    row, col = n % 16, n // 16
...
def _mfeat(S, MB, nblk, quad):
    ...
    ms = np.stack([S[:, MB == b][:, 0] for b in range(nblk)], 1)
    r = np.where(ms > 0.5, 1.0, OFF_REFLECT)
    F = [np.ones((len(r), 1)), r]
    if quad:
        F.append(
            np.stack(
                [r[:, i] * r[:, j] for i in range(nblk) for j in range(i + 1, nblk)], 1
            )
        )
    return np.hstack(F)
...
def structure_replace(pred, S, C, cfg, St, tcfg, teid):
    ...
    for lo, hi, MB, nblk, quad in [(17, 528, MB9, 9, True), (1, 16, MB4, 4, False)]:
        ...
        W = np.linalg.solve(
            Xtr.T @ Xtr + 1e-2 * np.eye(Xtr.shape[1]), Xtr.T @ ((C[trm] - m) @ B)
        )
...
def _torch():
    ...
    class Lift(nn.Module):  # learnable per-element complex reflection
        def __init__(s, n=256):
            super().__init__()
            s.a0 = nn.Parameter(torch.full((n,), 0.4))
            s.a1 = nn.Parameter(torch.full((n,), 1.0))
    ...
    class ResBlock(nn.Module):
        def __init__(s, ch):
            super().__init__()
            s.c1 = nn.Conv2d(ch, ch, 3, padding=1)
            s.c2 = nn.Conv2d(ch, ch, 3, padding=1)
            s.se = nn.Sequential(
                nn.Linear(ch, ch // 2), nn.GELU(), nn.Linear(ch // 2, ch), nn.Sigmoid()
            )

        def forward(s, x):
            h = F.gelu(s.c1(x))
            h = s.c2(h)
            g = s.se(h.mean((2, 3))).unsqueeze(-1).unsqueeze(-1)
            return F.gelu(x + h * g)

    class AttnBlock(nn.Module):
        def __init__(s, ch, heads=4):
            super().__init__()
            s.norm = nn.LayerNorm(ch)
            s.att = nn.MultiheadAttention(ch, heads, batch_first=True)
            s.ff = nn.Sequential(
                nn.LayerNorm(ch),
                nn.Linear(ch, 2 * ch),
                nn.GELU(),
        ...
        def forward(s, x):
            ...
            t = t + s.att(s.norm(t), s.norm(t), s.norm(t))[0]
            t = t + s.ff(t)
    ...
    class BigNet(nn.Module):
        def __init__(s, B, m, ch, blocks, mh, attn):
            super().__init__()
            s.lift = Lift()
            s.stem = nn.Conv2d(2, ch, 3, padding=1)
            s.blocks = nn.Sequential(*[ResBlock(ch) for _ in range(blocks)])
            s.attn = (
                nn.Sequential(*[AttnBlock(ch) for _ in range(attn)])
                if attn
                else nn.Identity()
            )
            s.head = nn.Conv2d(ch, 8, 1)
            s.bil = nn.Linear(512, K)
            s.mlp = nn.Sequential(nn.Linear(2048, mh), nn.GELU(), nn.Linear(mh, K))
            s.register_buffer("B", torch.tensor(B))
            s.register_buffer("mm", torch.tensor(m))

        def forward(s, r):
            L = s.lift(r)
            x = F.gelu(s.stem(L.view(-1, 2, 16, 16)))
            x = s.blocks(x)
            x = s.attn(x)
...
def cnn_params(cf):
    ...
    net = BigNet(
        np.zeros((484, K), np.float32),
        np.zeros(484, np.float32),
        cf["ch"],
        cf["blocks"],
        cf["mh"],
        cf["attn"],
...
# 484*K SVD basis is shared. Counted toward the per-predictor budget.
...
def load(c):
    ...
    teid = np.load(f"{DATA}/test_{c}_eid.npy", allow_pickle=True)
...
def apply_blend(c, cnn, q, m, B, blend):
    ...
    if bw.get("mode") == "convex":
        a = np.asarray(bw["a"], np.float64)
        cnk = (cnn - m) @ B
        qk = (q - m) @ B
        coeff = np.stack(
            [a[k] * qk[:, k] + (1 - a[k]) * cnk[:, k] for k in range(K)], 1
        )
        return m + coeff @ B.T
...
COLS = (
    ["example_id"]
    + [f"csi_real_{i:03d}" for i in range(242)]
...
# ----------------------------------------------------------------------------
# reproduce: saved weights + data  ->  submission.csv
# ----------------------------------------------------------------------------
def reproduce():
    import torch
    ...
    def build(sd):  # rebuild a BigNet from its state_dict
        ...
        mh = sd["mlp.0.weight"].shape[0]
        nb = len(
            [k for k in sd if k.startswith("blocks.") and k.endswith(".c1.weight")]
        )
    ...
    for c in CONDS:
        ...
        print(
            f"  {c:13s} {len(teid):>5} configs  (+{nrep} structured refined)",
            flush=True,
        )
...
# ----------------------------------------------------------------------------
# train: fit all 8 predictors from scratch-> save .pth, then reproduce
# ----------------------------------------------------------------------------
def train():
    torch, nn, F, Lift, ResBlock, AttnBlock, BigNet = _torch()
    ...
    def train_seed(S, C, B, m, cf, seed):
        ...
        va, fi = idx[:800], idx[800:]
        Rt = torch.tensor(S, device=DEV)
        ...
        for e in range(ep):
            net.train()
            perm = fi[rng.permutation(len(fi))]
    ...
    for c in CONDS:
        ...
        B, m = svd_basis(C)
        Bf, mf = B.astype(np.float32), m.astype(np.float32)
        seeds = [train_seed(S, Cf, Bf, mf, cf, sd) for sd in range(1, cf["seeds"] + 1)]
        models[c] = {
            "cnn_seeds": seeds,
            "B": torch.tensor(Bf),
            "mean": torch.tensor(mf),
        }
        print(
            f"  {c:13s} trained {cf['seeds']} seeds ({tot:,} params <=1M, incl. macro-block head)",
            flush=True,
        )
    os.makedirs("models", exist_ok=True)
    torch.save(
        {"models": models, "blend": {}}, MODEL_PATH
    )  # scalar-alpha blend by default
...
def params():
    print(
        f"{'predictor':9s} {'1 net':>10s} x seeds + quad + mblock = total   <= 1,000,000"
    )
    for k in ("Log", "Dipole"):
        cf = CFG[k]
        one = cnn_params(cf)
        tot = cf["seeds"] * one + QUAD_PARAMS + MBLOCK_PARAMS
        print(
            f"{k:9s} {one:>10,} x{cf['seeds']} + quad {QUAD_PARAMS:,} + mblock {MBLOCK_PARAMS:,} = {tot:>9,}   "
