...
is also serialized to models/the_silent_subcarriers_1.pkl; the submission can be rebuilt either by refitting from the bundled
...
# The 8 channel conditions of the task
...
def load_task1_csv(path: Path) -> Task1Data:
    ...
    real = np.array(
        [[float(row[col]) for col in real_cols] for row in rows], dtype=np.float64
    )
    imag = np.array(
        [[float(row[col]) for col in imag_cols] for row in rows], dtype=np.float64
    )
    target = None
    if "mi_bits" in fieldnames:
        target = np.array([float(row["mi_bits"]) for row in rows], dtype=np.float64)
    return Task1Data(
        rows=rows, fieldnames=fieldnames, h=real + 1j * imag, target=target
    )
...
def predict(
    ...
    model = fit_ridge_residual(
        train_features, train_baseline, train.target, train_mask, lam=lam
    )
...
# shared ridge-residual correction fit on the random-group training rows.
...
def save_models(
    data_dir: Path, out_path: Path, lam: float, residual_scale: float
) -> None:
    ...
    artifact = {
        ...
        "note": (
            "One shared estimator serves all 8 conditions; per-condition entries "
    ...
    print(
        f"saved 8-condition model artifact ({model.num_parameters()} params/model) -> {out_path}"
    )
...
def load_model(path: Path) -> RidgeResidualModel:
    ...
    return RidgeResidualModel(
        lam=st["lam"], beta=st["beta"], mean=st["mean"], scale=st["scale"]
    )


def predict_with_model(
    model: RidgeResidualModel, test: Task1Data, residual_scale: float
) -> np.ndarray:
...
def grouped_config_cv(
    data: Task1Data, lam: float, residual_scale: float, seed: int = 7
) -> float:
    ...
    fold, so no configuration is ever both trained on and validated on.
    """
    ...
    for fold in folds:
        ...
        pred[valid_mask] = baseline[
            valid_mask
        ] + residual_scale * model.predict_residual(features[valid_mask])
...
def report_validation(train: Task1Data, lam: float, residual_scale: float) -> None:
    ...
    print("Grouped-config 5-fold CV on the 120 random training rows (RMSE, bits):")
    print(f"  closed-form plug-in only (scale=0.0)      : {base_cv:.6f}")
    print(f"  + ridge residual, scale=1.0 (CV-best)     : {unit_cv:.6f}")
    print(
        f"  + ridge residual, scale={residual_scale:<4} (SHIPPED)     : {shipped_cv:.6f}"
    )
    print(
        f"  fit-on-all-random train RMSE (optimistic) : {rmse(baseline[random_mask], train.target[random_mask]):.6f}"
    )
...
def write_submission(
    ...
    pred_by_id = {
        row["example_id"]: value for row, value in zip(test.rows, pred, strict=True)
    }
    missing = [
        row["example_id"] for row in sample_rows if row["example_id"] not in pred_by_id
    ]
    if missing:
        raise ValueError(
            f"Missing predictions for {len(missing)} sample IDs, first={missing[0]}"
        )
    ...
    with output.open("w", newline="") as fh:
        ...
        for row in sample_rows:  # preserve sample_submission.csv row order exactly
            ...
            writer.writerow(
                {"example_id": example_id, "mi_bits": f"{pred_by_id[example_id]:.15g}"}
            )
...
def print_parameters(data_dir: Path, lam: float) -> None:
    ...
    print(
        f"  Task 1 limit (spec): 8 models, <= 2,000,000 params each. {n} << 2,000,000  -> OK"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--params", action="store_true", help="Print the parameter count and exit."
    )
    parser.add_argument(
        "--save-models",
        action="store_true",
...
def main() -> None:
    ...
    if args.save_models:
        save_models(
            args.data_dir, args.models_path, args.lambda_ridge, args.residual_scale
        )
        return
